...
spacesof_x = { # later do a thing , "caluclate and adjust the x es by add or subtracting by the amount of days left in that month
    0: "8.64",
    1: "64.91",
    2: "126.94",
...
def get_top_chart_n_stuff_const():
...
def get_bottom_table_n_stuff():
    ...
    # Generate time strings in ascending order (top = newest/smallest, bottom = oldest/largest)
    # Pick 15 unique hours between 1-23, sort in ascending order
    hours = random.sample(range(1, 24), 15)
    hours.sort()  # Ascending: smallest first (newest at top)
    ...
    for hour in hours:
        if hour == 1:
            ...
            time_strings.append(f"{hour} hours ago")

    # Shuffle the problem names and take 15 random ones
    ...
    # Generate each row with alternating background
    # Order: top = newest (smallest hours), bottom = oldest (largest hours)
    for i in range(15):
        problem_name = selected_problems[i]
        time_str = time_strings[i]
        bg_class = "bg-fill-4 dark:bg-dark-fill-4" if i % 2 == 0 else ""
...
def get_image_dict(starting_month_number):
...
def create_all_labels_list_to_append(starting_month_number,month_name):
...
async def safe_goto(page, url, timeout=30000, max_retries=3):
    """
    Navigate to a URL without letting a single slow/failed load crash the run.
    Tries progressively looser wait strategies on each retry, since
    'networkidle' hangs/times out on many real-world pages (ads, trackers,
    websockets that never go quiet).
    """
    wait_strategies = ["networkidle", "load", "domcontentloaded"]

    for attempt in range(1, max_retries + 1):
        strategy = wait_strategies[min(attempt - 1, len(wait_strategies) - 1)]
        try:
            print(f"[{_ts()}] Attempt {attempt}/{max_retries} -> "
                  f"goto(wait_until='{strategy}')")
            await page.goto(url, wait_until=strategy, timeout=timeout)
            print(f"[{_ts()}] Page loaded successfully on attempt {attempt}.")
            return True
        except PlaywrightTimeoutError:
            print(f"[{_ts()}] Timeout on attempt {attempt} using '{strategy}'.")
        except Exception as e:
            # Covers things like ERR_NAME_NOT_RESOLVED, connection reset, etc.
            print(f"[{_ts()}] Navigation error on attempt {attempt}: {e}")
...
async def screenshot(url: str):
    async with async_playwright() as p:
        ...
        # Both init scripts run once per new document, before page JS executes.
        # (No need to register the webdriver patch twice like the original did.)
        ...
        print(f"[{_ts()}] Page created.")

        loaded = await safe_goto(page, url, timeout=30000, max_retries=3)

        if not loaded:
            print(f"[{_ts()}] Giving up on {url} after retries — not crashing.")
            await browser.close()
            return None
        ...
        print(f"[{_ts()}] Dark mode applied: {is_dark}")

        # Do your scraping / screenshotting / whatever comes next here,
        # using `page` and `context` while the browser is still open.
        ...
        if await elements.count() == 0:
            print("don't exist")
            # populate it
            html_div = get_top_chart_n_stuff_const()
        ...
        print(month_number)
        # print(get_image_dict(month_number))

        print(create_all_labels_list_to_append(month_number, starting_month.title()))
        list_of_array = create_all_labels_list_to_append(month_number,
